    ...
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {addonsData.map((addon: any) => {
          // 🌟 คำนวณจำนวนสูงสุดที่กดได้: เอาค่าที่น้อยที่สุดระหว่าง สต็อก กับ โควต้า
          // ถ้า addonMaxLimit ไม่มีค่า ให้ใช้ addonQuantity แทน
          const limit = addon.addonMaxLimit || addon.addonQuantity;
          const maxAllowed = Math.min(addon.addonQuantity, limit);
          const currentCount = addonCounts[addon.addonID] || 0;
                ...
                <div className="flex items-end justify-between mt-auto pt-4 border-t border-slate-100">
                  <div>
                    <div className="font-black text-blue-600 text-2xl">
                      {addon.addonPrice.toLocaleString()} ฿ <span className="text-sm font-bold text-slate-500">/ วัน</span>
                    </div>
                    {/* 🌟 แสดงคำแนะนำโควต้า (ถ้ามี) */}
                    <div className="text-xs text-slate-400 mt-1">
                      เช่าได้สูงสุด {maxAllowed} ชิ้นต่อบิล
                    </div>
                  </div>
                  ...
                  <div className="flex items-center bg-slate-50 rounded-2xl p-1.5 border border-slate-200 shadow-inner">
                    {/* ปุ่มลบ */}
                    <button 
                      onClick={() => updateAddonCount(addon.addonID, -1, maxAllowed)} 
                    ...
                    {/* จำนวน */}
                    <span className="font-black text-slate-800 text-lg w-12 text-center">{currentCount}</span>
                    
                    {/* 🌟 ปุ่มบวก (เพิ่ม disabled ถ้าถึง limit) */}
                    <button 
                      onClick={() => updateAddonCount(addon.addonID, 1, maxAllowed)} 
                      disabled={currentCount >= maxAllowed}
                      className="w-10 h-10 flex items-center justify-center text-blue-600 hover:bg-white hover:shadow-sm rounded-xl transition-all font-bold text-xl cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      +
                    </button>
                  </div>
